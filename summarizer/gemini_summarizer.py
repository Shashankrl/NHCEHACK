    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        try:
            ...
            if gemini_models:
                ...
                self.model = genai.GenerativeModel(gemini_models[0].name)
                print(f"Using Gemini model: {gemini_models[0].name}")
            else:
                # Fallback to text-bison model if Gemini not available
                print("Gemini models not available, using a fallback model")
